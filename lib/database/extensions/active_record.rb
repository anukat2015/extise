class ActiveRecord::Base
  ...
  def self.findable?(object, options = {})
    ...
    return true if primary_key_class && !polymorphic && object.is_a?(primary_key_class)
    ...
    object.each { |item| return false if !item.is_a?(ActiveRecord::Base) && (primary_key_class && !polymorphic ? !item.is_a?(primary_key_class) : true) }
    return true
  end
end
